...
[The prerequisite audit](application-packaging-prerequisite-r7.md) confirms missing application qualification evidence and host release infrastructure, not a demonstrated Core runtime defect. All six exact-pair `host_packaging` checks fail. The host workflows still pin older Core sources, and EnCAP's Windows workflow packages third-party FFmpeg. The macOS candidate evidence is ad-hoc signed and explicitly excludes production signing and updates. Completing this requirement needs actual host migration and release-validation work; it cannot be satisfied by editing Core's ledger, substituting a generic harness or selecting downstream scope. ATIV and EnCAP were inspected read-only and were not changed.

The isolated historical late-cleanup failure is now a **residual qualification observation, not an active release blocker**, as directed by the owner on 2026-09-16. Its original source line and numeric error were not retained. An additional 3,000 full historical cycles and 1,000 reduced cycles did not reproduce it. [The retained evidence](evidence/windows-historical-cleanup-r7.json) remains explicit; neither a passing test nor a proven cause is retrospectively assigned to the original failure. Further investigation requires concrete evidence of a current reproducible defect.

The intended identifiers remain runtime `ffmpeg-9.0.1-r7` and Core `v0.2.2`; neither has been published. **There is no release commit or published asset checksum.** The table above identifies existing qualified candidates only. A fresh complete final matrix from the exact chosen release commit, publication, and consumer-facing download verification have not been performed in this finalization attempt because the prerequisite cannot be completed within the permitted scope. After that prerequisite is satisfied, qualify the final commit and promote those exact archives without rebuilding FFmpeg. The final release must contain `manifest.json`, `SHA256SUMS`, six runtime/source pairs, qualification receipts and GitHub provenance attestations.
