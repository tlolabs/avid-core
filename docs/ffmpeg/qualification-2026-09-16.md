...
The release policy decision remains pending: retain application packaging as a prerequisite, or explicitly authorize a Core-only runtime release while preserving those application checks as downstream requirements. Separately, the isolated historical raw-cleanup failure is still being investigated; it is not hidden by the corrected stress result.
